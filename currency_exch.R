...
data[LOCATION == 'CHN'] %>% ggplot() +
  aes(TIME, Value) +
  geom_line() +
  geom_ribbon(mapping = aes(ymin = 6, ymax = 7), alpha = .1) +
  # geom_vline(xintercept = 2008, linetype = 'dashed') +
  scale_x_continuous(c(1950, 2018), breaks = seq(1950, 2018, 5)) +
  scale_y_continuous(c(0, 10), breaks = seq(0, 10, 2)) +
  theme(panel.background = element_rect(fill = 'white'),
        legend.position = 'none')
...
chnusd %>% ggplot() +
  aes(date, value) +
  geom_line() +
  geom_ribbon(mapping = aes(ymin = 6, ymax = 7), alpha = .2) +
  labs(
